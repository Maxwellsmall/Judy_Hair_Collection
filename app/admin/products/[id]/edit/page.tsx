...
export default function EditProductPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        if (productRes.success && productRes.data?.product) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Keep existing confirmed URLs (non-blob), append newly uploaded
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      <div className="mb-6">
        ...
        <p className="text-sm text-neutral-500 mt-1">Update the product details below.</p>
      </div>
